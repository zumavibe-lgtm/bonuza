...
class LoginFrame(tk.Frame):
    ...
    def build_ui(self):
        ...
        self.user_entry = tk.Entry(self); self.user_entry.grid(row=0, column=1, padx=10, pady=5)
        tk.Label(self, text="Contraseña:").grid(row=1, column=0, padx=10, pady=5)
        self.pwd_entry = tk.Entry(self, show="*"); self.pwd_entry.grid(row=1, column=1, padx=10, pady=5)

        btn_login = tk.Button(self, text="Ingresar (F1)", command=self.attempt_login)
        btn_login.grid(row=2, column=0, columnspan=2, pady=10)

        self.master.bind("<F1>", lambda e: self.attempt_login())
        self.master.bind("<F12>", lambda e: self.master.destroy())
